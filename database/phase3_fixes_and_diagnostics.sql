-- ============================================
-- PHASE 3: FIXES, VERIFICATIONS `& DIAGNOSTICS
-- Generated 2025-11-26 11:01:21
-- ============================================

-- [START FILE: fix_session_constraint.sql]
...
-- [END FILE: fix_session_constraint.sql]


-- [START FILE: fix_duplicate_function.sql]
...
-- Verify the function was created (commented - schema compatibility)
-- SELECT routine_name, data_type, parameter_name, ordinal_position
-- FROM information_schema.parameters
-- WHERE specific_schema = 'public' AND routine_name = 'activate_subscription'
-- ORDER BY ordinal_position;
SELECT 'activate_subscription function created!' as status;


-- [END FILE: fix_duplicate_function.sql]


-- [START FILE: supabase_complete_migration.sql]
...
INSERT INTO payments (
  ...
  (SELECT id FROM coaches WHERE email = 'test@fitnessguru.com'),
  19.00,
  'PLN',
  'succeeded',
  'pi_test_123456789',
...
-- ✅ Added subscription columns to coaches
...
-- [END FILE: supabase_complete_migration.sql]


-- [START FILE: supabase_correct_migration.sql]
...
-- [END FILE: supabase_correct_migration.sql]


-- [START FILE: supabase_fixed_migration.sql]
...
-- If coaches doesn't exist, it might be called 'profiles' or 'users'

-- Try to add columns to coaches (if it exists)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'coaches') THEN
        ...
        RAISE NOTICE 'Added subscription columns to coaches table';
    ELSE
        RAISE NOTICE 'coaches table does not exist. Please check your table names first.';
    END IF;
END $$;
...
-- Add foreign key constraint only if coaches exists and constraint doesn't exist
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'coaches') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_payments_coach_id') THEN
        ALTER TABLE payments ADD CONSTRAINT fk_payments_coach_id 
        FOREIGN KEY (coach_id) REFERENCES coaches(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added foreign key constraint to payments table';
    ELSE
        RAISE NOTICE 'Skipped foreign key constraint - already exists or coaches table not found';
    END IF;
END $$;
...
-- 3. If coaches doesn't exist, tell me what your user table is called
-- 4. I'll create a specific migration for your table structure

SELECT 'Migration completed! Check the results above.' as status;

-- [END FILE: supabase_fixed_migration.sql]


-- [START FILE: check_tables.sql]
...
-- [END FILE: check_tables.sql]


-- [START FILE: check_coaches_structure.sql]
...
-- [END FILE: check_coaches_structure.sql]


-- [START FILE: check_payments_structure.sql]
...
-- [END FILE: check_payments_structure.sql]


-- [START FILE: check-your-account.sql]
...
-- [END FILE: check-your-account.sql]
